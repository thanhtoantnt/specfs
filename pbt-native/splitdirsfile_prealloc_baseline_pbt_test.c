/*
 * Property-based tests for splitDirsFile() in
 * eval/pre_alloc/baseline/util.c
 *
 * Oracle: Algebraic - Invariant (4d) plus Negative/Error Contract (4e)
 * Stronger considered:
 *   - Round-trip (4a): rejected - there is no inverse join helper in this SUT
 *   - Differential (7): rejected - no independent reference implementation
 *   - State Machine (3): rejected - splitDirsFile is a pure helper with no lifecycle
 * Weaker available: Metamorphic (4c), Crash-Only (6)
 *
...
{
    /* Printable ASCII except '/', which is the separator. */
    char c;
    do {
...
static void
build_noisy_path(struct theft *t, struct split_path_instance *sp)
{
    size_t off = 0;
    sp->path[0] = '\0';
...
static void
build_canonical_path(const struct split_path_instance *sp,
...
split_path_alloc_cb(struct theft *t, void *env, void **instance)
{
    (void)env;
    struct split_path_instance *sp = malloc(sizeof(*sp));
...
static void
split_path_free_cb(void *instance, void *env)
{
    (void)env;
    free(instance);
}
...
static theft_hash
split_path_hash_cb(const void *instance, void *env)
{
    (void)env;
    const struct split_path_instance *sp = instance;
...
static void
split_path_print_cb(FILE *f, const void *instance, void *env)
{
    (void)env;
    const struct split_path_instance *sp = instance;
    fprintf(f, "path=");
...
static struct theft_type_info split_path_info = {
    .alloc = split_path_alloc_cb,
    .free = split_path_free_cb,
    .hash = split_path_hash_cb,
    .print = split_path_print_cb,
};

static void
init_result(struct split_result *r)
{
...
static void
cleanup_result(struct split_result *r)
{
    free_dirs(r->dirs);
}

static void
run_split(const char *path, struct split_result *r)
{
    init_result(r);
    splitDirsFile(path, r->dirs, r->filename);
}

static int
result_matches_components(const struct split_result *r,
                          const struct split_path_instance *sp,
                          const char *context)
{
    if (sp->count == 0) {
        if (r->filename[0] != '\0') {
            fprintf(stderr, "%s: expected empty filename, got '%s'\n",
                    context, r->filename);
            return 0;
        }
        if (r->dirs[0] != NULL) {
            fprintf(stderr, "%s: expected dirname[0] == NULL for empty path\n",
                    context);
            return 0;
        }
        return 1;
    ...
    if (strcmp(r->filename, sp->components[sp->count - 1]) != 0) {
        fprintf(stderr, "%s: filename mismatch: got '%s', expected '%s'\n",
                context, r->filename, sp->components[sp->count - 1]);
        return 0;
    }

    for (size_t i = 0; i + 1 < sp->count; i++) {
        if (r->dirs[i] == NULL) {
...
static int
results_equal(const struct split_result *a, const struct split_result *b,
              const struct split_path_instance *sp)
{
    if (strcmp(a->filename, b->filename) != 0) return 0;
    for (size_t i = 0; i < sp->count; i++) {
        if ((a->dirs[i] == NULL) != (b->dirs[i] == NULL)) return 0;
        if (a->dirs[i] != NULL && strcmp(a->dirs[i], b->dirs[i]) != 0) return 0;
    }
    return a->dirs[sp->count] == NULL && b->dirs[sp->count] == NULL;
}

/* Property 1: tokenization matches the generated non-empty components. */
static enum theft_trial_res
prop_split_matches_components(struct theft *t, void *arg1)
{
    (void)t;
    const struct split_path_instance *sp = arg1;
...
/* Property 2: redundant leading/trailing/repeated slashes do not affect output. */
static enum theft_trial_res
prop_redundant_slashes_equivalent(struct theft *t, void *arg1)
{
    ...
    struct split_result noisy;
    struct split_result plain;
...
/* Property 3: splitDirsFile treats its const input as read-only. */
static enum theft_trial_res
...
/* Property 4: output strings are copies and survive later input overwrites. */
static enum theft_trial_res
...
/* Property 5: splitDirsFile must terminate dirname[] without relying on pre-zeroing. */
static enum theft_trial_res
prop_terminates_uninitialized_output(struct theft *t, void *arg1)
{
    ...
    char *dirs[MAX_PATH_LEN];
    char filename[MAX_FILE_LEN];
    size_t term = sp->count == 0 ? 0 : sp->count - 1;
    ...
    splitDirsFile(sp->path, dirs, filename);
...
main(void)
{
    ...
    printf("splitDirsFile pre_alloc baseline property-based tests:\n");
    ...
    RUN_PROP("terminates_uninitialized_output", prop_terminates_uninitialized_output, 300);
