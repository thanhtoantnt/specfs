 ...
 *
 * Oracle: Algebraic -- invariant/reference model for the read contract.
 * Stronger considered:
 *   - State Machine: rejected -- inode_read is a single-call operation with no lifecycle state transition.
 *   - Differential: rejected -- no independent implementation of inline_data inode_read is available in this target.
 ...
 * file_read so inode_read's return contract, low-level call, and copied
 * contents can be checked directly across inline-data boundaries.
 */
#include <theft.h>
...
static enum theft_alloc_res read_case_alloc_cb(struct theft *t, void *env, void **instance)
{
    ...
    case 4:
        ...
        break;
    case 5:
        rc->size = boundary_near(t, INLINE_DATA_SIZE, 512U);
        if (rc->size > MODEL_CAP) rc->size = MODEL_CAP;
        rc->offset = rc->size == 0 ? 0 : bounded_choice(t, rc->size - 1U);
        rc->len = bounded_choice(t, 1024U);
        break;
    case 6:
        rc->size = boundary_near(t, INLINE_DATA_SIZE, 512U);
        if (rc->size > MODEL_CAP) rc->size = MODEL_CAP;
        rc->offset = boundary_near(t, INLINE_DATA_SIZE, 512U);
        if (rc->offset > MODEL_CAP) rc->offset = MODEL_CAP;
        rc->len = bounded_choice(t, 1024U);
        break;
    default:
...
static enum theft_trial_res prop_empty_reads_return_null_buffer(struct theft *t, void *arg1)
{
    ...
    if (!ok) {
        fprintf(stderr,
                "empty read failed: size=%u offset=%u len=%u num=%u buf=%p calls=%u malloc_calls=%u last_len=%u\n",
                rc->size, rc->offset, rc->len, ret->num, (void *)ret->buf,
                observed.calls, malloc_buffer_calls, malloc_buffer_last_len);
    }
...
static enum theft_trial_res prop_inline_boundary_reads_match_model(struct theft *t, void *arg1)
{
    (void)t;
    const struct read_case *rc = arg1;
    if (rc->size <= INLINE_DATA_SIZE || rc->offset >= INLINE_DATA_SIZE) {
        return THEFT_TRIAL_SKIP;
    }
    unsigned expected = expected_num(rc->size, rc->offset, rc->len);
    if (expected == 0 || rc->offset + expected <= INLINE_DATA_SIZE) {
        return THEFT_TRIAL_SKIP;
    }
    ...
    int ok = ret->num == expected && ret->buf != NULL &&
             observed.calls == 1 && observed.offset == rc->offset &&
             observed.len == expected &&
             memcmp(ret->buf, model.bytes + rc->offset, expected) == 0;
    if (!ok) {
        fprintf(stderr,
                "inline boundary read failed: size=%u offset=%u len=%u num=%u expected=%u boundary=%u\n",
                rc->size, rc->offset, rc->len, ret->num, expected,
                (unsigned)INLINE_DATA_SIZE);
    }
...
int main(void)
{
    ...
    printf("inline_data inode_read property-based tests:\n");
    ...
    RUN_PROP("inline_boundary_reads_match_model", prop_inline_boundary_reads_match_model, 500);
