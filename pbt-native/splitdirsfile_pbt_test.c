 ...
 *
 * Oracle: splitDirsFile(path, dirname, filename) is a path-tokenization helper.
 * For valid NUL-terminated paths whose non-empty components are each shorter
 * than MAX_FILE_LEN and whose component count fits MAX_PATH_LEN, it should:
 *   - ignore empty components produced by repeated/leading/trailing '/';
 *   - copy the last non-empty component into filename;
 *   - allocate/copy preceding components into dirname[0..n-2];
 *   - terminate dirname with NULL;
 *   - leave the input path bytes unchanged and not alias outputs to the input.
 */
#include <theft.h>
...
main(void)
{
    ...
    printf("splitDirsFile property-based tests:\n");
