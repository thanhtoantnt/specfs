 ...
 *
 * The low-level file operation is stubbed so the properties observe only the
 * public inode_write contract: return length, inode size, and write call shape.
 */
#include <theft.h>
...
static enum theft_trial_res prop_out_of_range_offsets_are_noops(struct theft *t, void *arg1)
{
    ...
    unsigned actual = inode_write(&node, wc->buffer, wc->len, wc->offset);
    if (actual != 0 || observed.calls != 0 || node.size != wc->initial_size) {
        fprintf(stderr, "out_of_range_noop failed: actual=%u calls=%u size=%u initial=%u\n",
                actual, observed.calls, node.size, wc->initial_size);
        return THEFT_TRIAL_FAIL;
    }
    return THEFT_TRIAL_PASS;
}
...
static enum theft_trial_res prop_size_never_exceeds_max_file_size(struct theft *t, void *arg1)
{
    ...
    if (node.size > MAX_FILE_SIZE) {
        fprintf(stderr, "size exceeded max: size=%u max=%u offset=%u len=%u\n",
...
static enum theft_trial_res prop_file_write_matches_effective_write(struct theft *t, void *arg1)
{
    ...
    unsigned actual = inode_write(&node, wc->buffer, wc->len, wc->offset);
    unsigned expected = expected_actual_len(wc->len, wc->offset);
    if (expected == 0) return observed.calls == 0 ? THEFT_TRIAL_PASS : THEFT_TRIAL_FAIL;

    if (observed.calls != 1 || observed.node != &node || observed.offset != wc->offset ||
        observed.len != actual || observed.data != wc->buffer || actual != expected) {
        fprintf(stderr, "file_write mismatch: calls=%u offset=%u len=%u actual=%u expected=%u\n",
                observed.calls, observed.offset, observed.len, actual, expected);
        return THEFT_TRIAL_FAIL;
    }
    return THEFT_TRIAL_PASS;
}
...
int main(void)
{
    ...
    printf("pre_alloc inode_write property-based tests:\n");
