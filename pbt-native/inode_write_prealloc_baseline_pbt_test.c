 ...
 * contract from inode_management.c's generated prompt. A successful write must
 * return the effective byte count clamped to MAX_FILE_SIZE, call file_write once
 * with exactly that effective range, and update inode size without exceeding
 * MAX_FILE_SIZE. Invalid offsets and NULL inputs must be no-ops.
 *
 * The low-level file operation is stubbed so the properties exercise the real
 * baseline inode_write symbol while observing only the public call contract.
 */
#include <theft.h>
...
static unsigned expected_size_after(unsigned initial_size, unsigned offset, unsigned actual_len)
{
    if (actual_len == 0U) return initial_size;
    unsigned end = offset + actual_len;
    return end > initial_size ? end : initial_size;
}
...
static enum theft_trial_res prop_size_updates_to_effective_end(struct theft *t, void *arg1)
{
    (void)t;
    const struct write_case *wc = arg1;
    struct inode node = make_inode(wc->initial_size);
    ...
    unsigned actual = inode_write(&node, wc->buffer, wc->len, wc->offset);
    unsigned expected_len = expected_actual_len(wc->len, wc->offset);
    unsigned expected_size = expected_size_after(wc->initial_size, wc->offset, expected_len);

    if (actual != expected_len || node.size != expected_size || node.size > MAX_FILE_SIZE) {
        fprintf(stderr,
                "size_update failed: actual=%u expected_len=%u size=%u expected_size=%u max=%u offset=%u len=%u\n",
                actual, expected_len, node.size, expected_size, MAX_FILE_SIZE,
                wc->offset, wc->len);
        return THEFT_TRIAL_FAIL;
    }
    return THEFT_TRIAL_PASS;
...
static enum theft_trial_res prop_out_of_range_offsets_are_noops(struct theft *t, void *arg1)
{
    ...
    unsigned actual = inode_write(&node, wc->buffer, wc->len, wc->offset);
    if (actual != 0U || observed_write.calls != 0U || node.size != wc->initial_size) {
        fprintf(stderr,
                "out_of_range_noop failed: actual=%u calls=%u size=%u initial=%u offset=%u len=%u\n",
                actual, observed_write.calls, node.size, wc->initial_size,
                wc->offset, wc->len);
        return THEFT_TRIAL_FAIL;
    }
    return THEFT_TRIAL_PASS;
...
static enum theft_trial_res prop_null_inputs_are_noops(struct theft *t, void *arg1)
{
    ...
    if (null_node != 0U || null_buffer != 0U ||
        observed_write.calls != 0U || node.size != wc->initial_size) {
        fprintf(stderr,
                "null no-op failed: null_node=%u null_buffer=%u calls=%u size=%u initial=%u\n",
                null_node, null_buffer, observed_write.calls, node.size,
                wc->initial_size);
        return THEFT_TRIAL_FAIL;
    }
    return THEFT_TRIAL_PASS;
...
int main(void)
{
    ...
    printf("pre_alloc baseline inode_write property-based tests:\n");
    RUN_PROP("return_is_clamped", prop_return_is_clamped, 500);
    RUN_PROP("size_updates_to_effective_end", prop_size_updates_to_effective_end, 500);
