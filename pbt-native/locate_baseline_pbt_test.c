 ...
 * eval/extent/baseline/path_handling.c
 *
 * Oracle: reference path traversal, lock-coupling contract, and NULL-input
 * safety. Starting from a locked inode, an empty path returns the original inode
 * still locked; a fully present path returns the final inode locked and unlocks
 * intermediates; a miss returns NULL and releases every lock held by the
 * traversal. NULL API inputs should return NULL without crashing. Directory
 * lookup is checked against the generated tree shape rather than the
 * implementation.
 */
#include <theft.h>
...
static enum theft_alloc_res null_locate_case_alloc_cb(struct theft *t, void *env, void **instance)
{
    (void)env;
    struct null_locate_case *nc = malloc(sizeof(*nc));
    if (nc == NULL) return THEFT_ALLOC_ERROR;
    memset(nc, 0, sizeof(*nc));

    switch (theft_random_choice(t, 4U)) {
    case 0: /* NULL path with a valid current inode. */
        nc->null_cur = 0;
        nc->null_path = 1;
        nc->nonempty_path = 0;
        break;
    case 1: /* NULL current inode with an empty path. */
        nc->null_cur = 1;
        nc->null_path = 0;
        nc->nonempty_path = 0;
        break;
    case 2: /* NULL current inode with a non-empty path. */
        nc->null_cur = 1;
        nc->null_path = 0;
        nc->nonempty_path = 1;
        break;
    default: /* Both API inputs are NULL. */
        nc->null_cur = 1;
        nc->null_path = 1;
        nc->nonempty_path = 0;
        break;
...
static void null_locate_case_free_cb(void *instance, void *env)
{
    (void)env;
    free(instance);
}

static theft_hash null_locate_case_hash_cb(const void *instance, void *env)
{
    (void)env;
    return theft_hash_onepass(instance, sizeof(struct null_locate_case));
}

static void null_locate_case_print_cb(FILE *f, const void *instance, void *env)
{
    (void)env;
    const struct null_locate_case *nc = instance;
    fprintf(f, "{null_cur=%d, null_path=%d, nonempty_path=%d}",
            nc->null_cur, nc->null_path, nc->nonempty_path);
}

static struct theft_type_info null_locate_case_info = {
    .alloc = null_locate_case_alloc_cb,
    .free = null_locate_case_free_cb,
    .hash = null_locate_case_hash_cb,
    .print = null_locate_case_print_cb,
};
...
static enum theft_trial_res prop_null_inputs_return_null_without_crash(struct theft *t, void *arg1)
{
    (void)t;
    const struct null_locate_case *nc = arg1;
    pid_t pid = fork();
    if (pid < 0) return THEFT_TRIAL_ERROR;
    ...
    if (pid == 0) {
        ...
        char *empty_path[] = { NULL };
        char *nonempty_path[] = { (char *)"missing", NULL };
        struct inode *cur = nc->null_cur ? NULL : &node;
        char **path = nc->null_path ? NULL : (nc->nonempty_path ? nonempty_path : empty_path);

        struct inode *ret = locate(cur, path);
        _exit(ret == NULL ? 0 : 10);
    }

    int status = 0;
    if (waitpid(pid, &status, 0) < 0) return THEFT_TRIAL_ERROR;
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) return THEFT_TRIAL_PASS;
    return THEFT_TRIAL_FAIL;
}
...
#define RUN_PROP(name_, prop_, trials_) \
    RUN_PROP_WITH_TYPE(name_, prop_, &locate_case_info, trials_)
...
int main(void)
{
    ...
    RUN_PROP_WITH_TYPE("null_inputs_return_null_without_crash",
                       prop_null_inputs_return_null_without_crash,
                       &null_locate_case_info, 100);
    printf("\n%d failure(s)\n", failures);
