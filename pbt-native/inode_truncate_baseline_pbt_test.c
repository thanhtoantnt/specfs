 ...
 * contract. Stronger state-machine testing is unnecessary because
 * inode_truncate is a single-call mutation whose observable state is node->size
 * plus the low-level allocate/clear calls. The low-level file operations are
 * stubbed so these properties exercise the real baseline inode_truncate symbol
 * while observing the requested range contract precisely.
 */
#include <theft.h>
...
static enum theft_trial_res prop_shrink_clears_removed_tail(struct theft *t, void *arg1)
{
    ...
    int ok = observed.clear_calls == 1U && observed.clear_node == &node &&
             ...
             observed.allocate_calls == 0U && observed.write_calls == 0U;
    if (!ok) {
        fprintf(stderr,
                "shrink_clears_removed_tail failed: old=%u requested=%u "
                "clear=(calls=%u,start=%u,len=%u) allocs=%u writes=%u\n",
                tc->old_size, tc->requested_size, observed.clear_calls,
...
static enum theft_trial_res prop_grow_allocates_and_clears_new_region(struct theft *t, void *arg1)
{
    ...
    int ok = observed.allocate_calls == 1U && observed.allocate_tb == &file &&
    ...
    if (!ok) {
        fprintf(stderr,
                "grow_allocates_and_clears_new_region failed: old=%u requested=%u "
                "alloc=(calls=%u,offset=%u,len=%u) clear=(calls=%u,start=%u,len=%u) writes=%u\n",
                tc->old_size, tc->requested_size, observed.allocate_calls,
                observed.allocate_offset, observed.allocate_len, observed.clear_calls,
                observed.clear_start, observed.clear_len, observed.write_calls);
    }
    return ok ? THEFT_TRIAL_PASS : THEFT_TRIAL_FAIL;
}
...
static enum theft_trial_res prop_same_size_is_noop_except_size_assignment(struct theft *t, void *arg1)
{
    ...
    int ok = node.size == tc->old_size && observed.allocate_calls == 0U &&
             observed.clear_calls == 0U && observed.write_calls == 0U;
    if (!ok) {
        fprintf(stderr,
                "same_size_noop failed: old=%u requested=%u size=%u allocs=%u clears=%u writes=%u\n",
...
int main(void)
{
    ...
    printf("baseline extent inode_truncate property-based tests:\n");
