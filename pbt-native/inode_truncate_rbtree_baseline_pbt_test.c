 ...
 * contract. inode_truncate is a single-call state mutation; the observable
 * contract is node->size and low-level clear/write/allocation side effects.
 * These tests link the real rbtree baseline inode_truncate implementation and
 * stub only collaborators so the requested truncate range can be observed.
 */
#include <theft.h>
...
/* Oracle: growing exposes zeroed bytes by clearing [old_size, new_size). */
static enum theft_trial_res prop_grow_clears_new_region(struct theft *t, void *arg1)
{
    ...
    if (!ok) {
        fprintf(stderr,
                ...
                "clear=(calls=%u,start=%u,len=%u) reads=%u writes=%u\n",
                tc->old_size, tc->requested_size, node.size, observed.clear_calls,
                observed.clear_start, observed.clear_len, observed.read_calls,
                observed.write_calls);
    }
    return ok ? THEFT_TRIAL_PASS : THEFT_TRIAL_FAIL;
}
...
int main(void)
{
    ...
    printf("rbtree baseline inode_truncate property-based tests:\n");
