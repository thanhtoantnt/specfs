 ...
 *   - Differential: rejected, there is no independent delay_alloc constructor to compare against.
...
static enum theft_trial_res prop_nested_table_is_zeroed(struct theft *t, void *arg1)
{
    (void)t;
    const struct inode_case *ic = arg1;
    struct inode *node = malloc_inode(ic->mode, ic->maj, ic->min);
...
static enum theft_trial_res prop_allocations_do_not_alias(struct theft *t, void *arg1)
{
    ...
    if (ok) {
        ...
        if (ok) {
            if (ic->mode == DIR_MODE) {
                ok = first->dir != second->dir && first->file == NULL && second->file == NULL;
            } else {
                ok = first->file != second->file && first->dir == NULL && second->dir == NULL;
            }
        }
    }
...
{
    int failures = 0;
    printf("malloc_inode delay_alloc property-based tests:\n");
    RUN_PROP("constructs_expected_layout", prop_constructs_expected_layout, 500);
    RUN_PROP("nested_table_is_zeroed", prop_nested_table_is_zeroed, 500);
