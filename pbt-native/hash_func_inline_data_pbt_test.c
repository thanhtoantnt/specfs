/*
 * Property-based tests for hash_func() in eval/inline_data/optimization/hashing.c
 *
...
prop_matches_unsigned_byte_recurrence(struct theft *t, void *arg1)
{
    ...
    /* Oracle: reference recurrence from the embedded spec, interpreting each
     * filename byte like nat_of_ascii c (unsigned 0..255) and reducing mod 512.
     */
...
prop_output_is_9_bit(struct theft *t, void *arg1)
{
    (void)t;
    struct name_instance *ni = arg1;
    /* Oracle: embedded spec guarantee hash_func(name) is in [0, 511]. */
    unsigned int actual = hash_func((char *)ni->buf);
...
prop_deterministic(struct theft *t, void *arg1)
{
    (void)t;
    struct name_instance *ni = arg1;
    /* Oracle: embedded spec guarantee same input gives same output. */
...
prop_does_not_mutate_input(struct theft *t, void *arg1)
{
    ...
    /* Oracle: embedded spec guarantee hash_func has no side effects on name. */
...
prop_last_byte_extends_prefix(struct theft *t, void *arg1)
{
    ...
    /* Oracle: algebraic prefix law induced by hash = hash * 131 + current_byte,
     * with current_byte interpreted as an unsigned byte and final modulus 512.
     */
    unsigned int actual = hash_func((char *)ni->buf);
...
int main(void)
{
    ...
    printf("hash_func inline_data property-based tests:\n");
