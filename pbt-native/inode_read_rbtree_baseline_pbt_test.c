 ...
 * single-call query; stronger state-machine testing is unnecessary because the
 * observable behavior is the returned read_ret, absence of inode mutation, and
 * the exact low-level file_read range. This test links the real rbtree baseline
 * inode_read implementation and stubs only allocation, hashing, and low-level
 * I/O collaborators. A byte-array model backs file_read so clamping,
...
char *malloc_buffer(unsigned len)
{
    ...
    /* Keep overflow witnesses cheap and deterministic: the property checks the
     * requested length via malloc_buffer_last_len/observed_read.len, while the
     * file_read stub writes at most MODEL_CAP bytes into this scratch buffer. */
...
static enum theft_alloc_res read_case_alloc_cb(struct theft *t, void *env, void **instance)
{
    ...
    case 4:
        ...
        break;
    case 5:
        /* Explicit unsigned-overflow boundary: offset < size, but
         * offset + UINT_MAX wraps, so a correct implementation must use
         * size - offset instead of the overflowing sum to clamp the read. */
...
int main(void)
{
    ...
    printf("rbtree baseline inode_read property-based tests:\n");
