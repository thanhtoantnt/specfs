 ...
 * eval/inline_data/optimization/path_handling.c
 *
 * Oracle: Algebraic — Invariant (4d)
 * Stronger considered:
 *   - State Machine (3): rejected — getlen has no internal state or lifecycle
 *   - Differential (7): rejected — no second implementation of the same contract
 *   - Round-trip (4a): rejected — getlen is a one-way query with no inverse
...
static enum theft_trial_res prop_empty_path_returns_zero(struct theft *t, void *arg1)
{
    (void)t;
    struct getlen_case *tc = arg1;
...
{
    int failures = 0;
    printf("getlen inline_data property-based tests:\n");
    ...
    RUN_PROP("empty_path_returns_zero", prop_empty_path_returns_zero, 200);
    printf("\n%d failure(s)\n", failures);
