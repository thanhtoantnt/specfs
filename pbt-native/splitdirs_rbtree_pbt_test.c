...
prop_terminates_uninitialized_output(struct theft *t, void *arg1)
{
    ...
    splitDirs(sp->path, r.dirs);

    int ok = r.dirs[sp->count] == NULL;
    if (!ok) {
        fprintf(stderr, "terminates_uninitialized_output: dirname[%zu] was not set to NULL for path ",
                sp->count);
        print_escaped(stderr, sp->path);
        fprintf(stderr, "\n");
    }
...
main(void)
{
    ...
    printf("splitDirs rbtree property-based tests:\n");
