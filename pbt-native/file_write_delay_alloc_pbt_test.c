 ...
 *
 * Oracle: Algebraic - invariant/reference-model
 * Stronger considered:
 *   - State Machine (3): rejected - file_write is a single-call mutator; it has no explicit lifecycle state or state-branching API.
 *   - Differential (7): rejected - no second independently meaningful delayed-allocation implementation is present.
 *   - Round-trip (4a): rejected - write is lossy and has no true inverse.
