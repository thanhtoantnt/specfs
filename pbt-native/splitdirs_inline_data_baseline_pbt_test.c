 ...
 *
 * Oracle: Algebraic — Invariant / metamorphic tokenizer contract.
 * Stronger considered:
 *   - State Machine (3): rejected — splitDirs is a pure helper with no lifecycle/state field.
 *   - Differential (7): rejected — no independent production implementation exists.
 *   - Round-trip (4a): rejected — splitDirs discards redundant slash placement.
 * Weaker available: Crash-Only (6).
 *
 * splitDirs(path, dirname) tokenizes a slash-delimited path into
...
main(void)
{
    ...
    printf("splitDirs inline_data baseline property-based tests:\n");
