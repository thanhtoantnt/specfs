 ...
 *   - Differential: rejected — no independent implementation of the delayed-allocation read contract
...
struct read_case {
    ...
    unsigned char write_data[MAX_READ_BYTES];
};
...
static enum theft_alloc_res read_case_alloc_cb(struct theft *t, void *env, void **instance)
{
    ...
    case 0: rc->page_off = 0U; break;
    case 1: rc->page_off = 1U; break;
    case 2: rc->page_off = PAGE_SIZE - 1U; break;
    case 3: rc->page_off = PAGE_SIZE - 8U; break;
    case 4: rc->page_off = PAGE_SIZE / 2U; break;
    case 5: rc->page_off = PAGE_SIZE - 128U; break;
    default: rc->page_off = bounded_choice(t, PAGE_SIZE); break;
    }
    ...
    for (unsigned i = 0; i < sizeof(rc->write_data); i++) {
        rc->write_data[i] = (unsigned char)theft_random_choice(t, 256U);
    }
...
static void read_case_print_cb(FILE *f, const void *instance, void *env)
{
    ...
    fprintf(f,
            "{start_page=%u, page_count=%u, page_off=%u, len=%u, present_mask=0x%x}",
...
static int populate_index_pages(struct indextb *tb, const struct read_case *rc)
{
...
static unsigned char sparse_model_byte(const struct read_case *rc, unsigned absolute_offset)
{
...
/* Oracle: file_read must return exactly the bytewise reference projection over
 * allocated pages and must zero-fill every hole. */
static enum theft_trial_res prop_sparse_pages_match_reference(struct theft *t, void *arg1)
{
    (void)t;
...
/* Oracle: delayed writes buffered by file_write are immediately visible through
 * file_read, even before delalloc flushes them into tb->index. */
static enum theft_trial_res prop_buffered_write_is_read_back_exactly(struct theft *t, void *arg1)
{
    (void)t;
    const struct read_case *rc = arg1;
    struct indextb tb;
    ...
    unsigned offset = case_offset(rc);
    file_write(&tb, offset, rc->len, (const char *)rc->write_data);
    ...
    memset(out, 0x5A, rc->len);

    file_read(&tb, offset, rc->len, (char *)out);

    int ok = memcmp(out, rc->write_data, rc->len) == 0;
    if (!ok) {
        fprintf(stderr, "buffered write/read mismatch offset=%u len=%u\n", offset, rc->len);
    }
...
/* Oracle: reading an entirely unallocated file range returns zero bytes. */
static enum theft_trial_res prop_unallocated_range_zero_filled(struct theft *t, void *arg1)
{
    (void)t;
    const struct read_case *rc = arg1;
    struct indextb tb;
    ...
    for (unsigned i = 0; i < rc->len; i++) {
        if (out[i] != 0U) {
            fprintf(stderr, "unallocated read returned nonzero byte=%u value=%u\n", i, out[i]);
            ok = 0;
            break;
        }
    }
...
/* Oracle: a zero-length read is a no-op on the caller-provided destination. */
static enum theft_trial_res prop_zero_length_read_does_not_modify_output(struct theft *t, void *arg1)
{
    (void)t;
    const struct read_case *rc = arg1;
    struct indextb tb;
...
int main(void)
{
    ...
    printf("file_read delay_alloc property-based tests:\n");
    failures += run_prop("sparse_pages_match_reference", prop_sparse_pages_match_reference, 300);
    failures += run_prop("buffered_write_is_read_back_exactly", prop_buffered_write_is_read_back_exactly, 300);
    failures += run_prop("unallocated_range_zero_filled", prop_unallocated_range_zero_filled, 200);
    failures += run_prop("zero_length_read_does_not_modify_output", prop_zero_length_read_does_not_modify_output, 100);
