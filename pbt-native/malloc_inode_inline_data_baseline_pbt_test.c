 ...
 *   - Differential: rejected, there is no independent inline_data baseline constructor to compare against.
...
static int file_table_is_zeroed(const struct indextb *file)
{
    for (unsigned i = 0U; i < INDEXTB_NUM; i++) {
...
static int has_common_zeroed_state(const struct inode *node)
{
    return node->size == 0U &&
           node->mutex == 0 &&
           node->hd == NULL &&
...
static enum theft_trial_res prop_constructs_expected_layout(struct theft *t, void *arg1)
{
    ...
    if (ok) {
        ok = node->mode == (unsigned)ic->mode &&
             node->maj == ic->maj &&
             node->min == ic->min &&
             has_common_zeroed_state(node);

        if (ok) {
            if (ic->mode == DIR_MODE) {
                ok = node->dir != NULL && node->file == NULL;
            } else {
                ok = node->dir == NULL && node->file != NULL;
            }
        }
...
static enum theft_trial_res prop_mode_specific_table_is_zeroed(struct theft *t, void *arg1)
{
    (void)t;
    const struct inode_case *ic = arg1;
    struct inode *node = malloc_inode(ic->mode, ic->maj, ic->min);
    ...
    if (ok) {
        if (ic->mode == DIR_MODE) {
            ok = node->dir != NULL && node->file == NULL && dir_table_is_zeroed(node->dir);
        } else {
            ok = node->dir == NULL && node->file != NULL && file_table_is_zeroed(node->file);
        }
    }
...
static enum theft_trial_res prop_non_directory_modes_get_file_table(struct theft *t, void *arg1)
{
    (void)t;
    const struct inode_case *ic = arg1;
    struct inode *node = malloc_inode(ic->mode, ic->maj, ic->min);
    ...
    if (ok) {
        if (ic->mode != DIR_MODE) {
            ok = node->file != NULL && node->dir == NULL && file_table_is_zeroed(node->file);
        } else {
            ok = node->file == NULL && node->dir != NULL;
        }
    }
        ...
        if (ok) {
            if (ic->mode == DIR_MODE) {
                ok = first->dir != NULL && second->dir != NULL &&
                     ...
                     first->file == NULL && second->file == NULL;
            } else {
                ok = first->file != NULL && second->file != NULL &&
                     first->file != second->file &&
                     first->dir == NULL && second->dir == NULL;
            }
        }
    }
...
{
    int failures = 0;
    printf("malloc_inode inline_data baseline property-based tests:\n");
    RUN_PROP("constructs_expected_layout", prop_constructs_expected_layout, 500);
    RUN_PROP("mode_specific_table_is_zeroed", prop_mode_specific_table_is_zeroed, 500);
    RUN_PROP("non_directory_modes_get_file_table", prop_non_directory_modes_get_file_table, 500);
