 ...
 * byte-array model so these properties exercise the real inline_data baseline
 * inode_read symbol while checking clamping, delegation, contents, empty-read
 * behavior, and NULL-node safety.
 */
#include <theft.h>
...
static enum theft_trial_res prop_empty_reads_return_null_buffer(struct theft *t, void *arg1)
{
    ...
    if (!ok) {
        fprintf(stderr,
                "empty_reads_return_null_buffer failed: size=%u offset=%u len=%u num=%u buf=%p file_calls=%u malloc_calls=%u last_len=%u\n",
                rc->size, rc->offset, rc->len, ret->num, (void *)ret->buf,
                observed_read.calls, malloc_buffer_calls, malloc_buffer_last_len);
    }
...
int main(void)
{
    ...
    printf("baseline inline_data inode_read property-based tests:\n");
