 ...
 *
 * Oracle: state/invariant contract from lowlevel_file.c's generated prompt.
 * For any valid non-empty byte range, every intersecting page-table slot must
 * be allocated and zero-initialized. Reallocating an existing range must be
 * idempotent and preserve existing page pointers/content. Zero-length ranges
 * are no-ops.
 */
#include <theft.h>
...
/* lowlevel_file.c references cnt from file_read; provide the test stub here. */
...
{
    for (unsigned i = 0; i < PG_SIZE; i++) {
        page[i] = (unsigned char)(fill + (unsigned char)(page_index * 17U) +
                                  (unsigned char)(i * 31U));
    }
}
...
int main(void)
{
    ...
    printf("inline_data baseline file_allocate property-based tests:\n");
