 ...
 *   - Differential: rejected, there is no independent rbtree baseline constructor to compare against.
...
static int prealloc_data_is_zeroed(const struct inode *node)
{
    for (unsigned i = 0U; i < 512U; i++) {
        if (node->prealloc_data[i] != NULL) {
            return 0;
        }
    }
    return 1;
...
static int has_common_zeroed_state(const struct inode *node)
{
    return node->size == 0U &&
           ...
           node->extents == NULL &&
           node->preallocs == NULL &&
           node->prealloc_count == 0U &&
           prealloc_data_is_zeroed(node);
}
...
static enum theft_trial_res prop_directory_table_is_zeroed(struct theft *t, void *arg1)
{
    ...
    if (ok) {
        if (ic->mode == DIR_MODE) {
            ok = node->dir != NULL && dir_table_is_zeroed(node->dir);
        } else {
            ok = node->dir == NULL;
        }
    }
...
static enum theft_trial_res prop_non_directories_start_without_file_allocations(struct theft *t, void *arg1)
{
    (void)t;
    const struct inode_case *ic = arg1;
    struct inode *node = malloc_inode(ic->mode, ic->maj, ic->min);
...
static enum theft_trial_res prop_allocations_do_not_alias(struct theft *t, void *arg1)
{
    ...
    if (ok) {
        ...
        if (ok) {
            if (ic->mode == DIR_MODE) {
                ok = first->dir != NULL && second->dir != NULL && first->dir != second->dir;
            } else {
...
{
    int failures = 0;
    printf("malloc_inode rbtree baseline property-based tests:\n");
    ...
    RUN_PROP("non_directories_start_without_file_allocations", prop_non_directories_start_without_file_allocations, 500);
