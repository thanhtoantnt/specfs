...
static enum theft_trial_res prop_clear_zeros_target_span_and_preserves_rest(struct theft *t, void *arg1)
{
    ...
    unsigned char *expected = malloc(extent_bytes);
    if (expected == NULL) {
        destroy_inode(&node);
        return THEFT_TRIAL_ERROR;
    }
    memset(expected, cc->fill, extent_bytes);
    memset(expected + cc->page_off, 0, cc->len);

    clear_file(&node, offset, cc->len);

    int ok = 1;
    for (unsigned i = 0; i < extent_bytes; i++) {
        unsigned page = cc->start_page + (i / PG_SIZE);
        unsigned page_off = i % PG_SIZE;
        unsigned char actual = node.file->index[page][page_off];
        if (actual != expected[i]) {
            fprintf(stderr,
                    "clear span mismatch at byte=%u got=%u expected=%u offset=%u len=%u\n",
                    i, actual, expected[i], offset, cc->len);
            ok = 0;
            break;
        }
    }
...
static enum theft_trial_res prop_sparse_clear_allocates_zero_pages(struct theft *t, void *arg1)
{
    (void)t;
    const struct clear_case *cc = arg1;
    struct inode node = make_inode();
    ...
    for (unsigned page = start_page; page <= end_page; page++) {
        ...
        for (unsigned byte = 0; byte < PG_SIZE; byte++) {
            if (node.file->index[page][byte] != 0U) {
                fprintf(stderr,
                        "sparse clear returned non-zero byte page=%u off=%u value=%u\n",
                        page, byte, node.file->index[page][byte]);
                destroy_inode(&node);
                return THEFT_TRIAL_FAIL;
            }
        }
...
static enum theft_trial_res prop_zero_length_clear_is_noop(struct theft *t, void *arg1)
{
    ...
    for (unsigned i = 0; i < extent_bytes; i++) {
        unsigned page = cc->start_page + (i / PG_SIZE);
        unsigned page_off = i % PG_SIZE;
        before[i] = node.file->index[page][page_off];
    }
    ...
    for (unsigned i = 0; i < extent_bytes; i++) {
        unsigned page = cc->start_page + (i / PG_SIZE);
        unsigned page_off = i % PG_SIZE;
        after[i] = node.file->index[page][page_off];
    }
...
int main(void)
{
    ...
    printf("inline_data baseline clear_file property-based tests:\n");
    RUN_PROP("clear_zeros_target_span_and_preserves_rest",
             prop_clear_zeros_target_span_and_preserves_rest, 300);
    RUN_PROP("sparse_clear_allocates_zero_pages",
             prop_sparse_clear_allocates_zero_pages, 300);
    RUN_PROP("zero_length_clear_is_noop",
             prop_zero_length_clear_is_noop, 200);
    RUN_PROP("out_of_range_clear_is_noop",
             prop_out_of_range_clear_is_noop, 25);
