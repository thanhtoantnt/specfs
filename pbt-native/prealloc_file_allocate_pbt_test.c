 ...
 *
 * Oracle: state/invariant contract for pre-allocation-backed extent allocation.
 * For every valid non-empty byte range, all pages intersecting
 * [offset, offset + len) must become allocated and newly allocated bytes must
 * be zero-filled. Re-allocating an allocated range must be idempotent and
 * preserve bytes. Pages later served from the preallocation pool must
 * materialize without corrupting already allocated bytes. Empty or
 * out-of-range allocations are no-ops.
 */
#include <theft.h>
...
int main(void)
{
    ...
    printf("pre_alloc file_allocate property-based tests:\n");
