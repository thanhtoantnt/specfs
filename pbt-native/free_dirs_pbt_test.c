 ...
 *
 * Oracle: Algebraic - NULL-terminated deallocation invariant from the
 * free_dirs spec: for any heap array of heap strings terminated by the first
 * NULL sentinel, free_dirs must free each pointer before the sentinel exactly
 * once and must not touch any trailing pointers after the sentinel.
 * Stronger considered:
 *   - State Machine: rejected, free_dirs is a single-shot destructor with no
 *     meaningful post-state except allocator events.
...
#define MAX_FREE_EVENTS (MAX_PREFIX_COMPONENTS + MAX_TRAILING_COMPONENTS + 1U)
#define PREFIX_EVENT_KIND 1U
#define TRAILING_EVENT_KIND 2U
#define UNKNOWN_EVENT_KIND 3U
...
static enum theft_trial_res prop_empty_list_is_noop(struct theft *t, void *arg1)
{
    (void)t;
    const struct free_dirs_case *fc = arg1;
    struct runtime_dirs rt;
    ...
    int ok = observed_unknown_frees == 0U && event_count == 0U && no_trailing_frees(&rt);
    cleanup_remaining(&rt);
    return ok ? THEFT_TRIAL_PASS : THEFT_TRIAL_FAIL;
}
...
int main(void)
{
    ...
    printf("free_dirs delay_alloc property-based tests:\n");
    ...
    RUN_PROP("empty_list_is_noop", prop_empty_list_is_noop, 300);
