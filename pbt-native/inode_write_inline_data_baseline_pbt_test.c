 ...
 * with exactly that effective range, and keep inode size within MAX_FILE_SIZE.
 * Out-of-range offsets must be no-ops.
 *
 * The low-level file operations are stubbed so these properties exercise the
 * real inline_data baseline inode_write symbol while observing the public call
 * contract precisely.
 */
#include <theft.h>
...
static enum theft_trial_res prop_growth_is_allocated_and_cleared(struct theft *t, void *arg1)
{
    (void)t;
    const struct write_case *wc = arg1;
    if (wc->len == 0U && wc->offset < MAX_FILE_SIZE) return THEFT_TRIAL_PASS;
    ...
    (void)inode_write(&node, wc->buffer, wc->len, wc->offset);
    unsigned expected_actual = expected_actual_len(wc->len, wc->offset);
    unsigned expected_size = expected_size_after(wc->initial_size, wc->offset, expected_actual);
    unsigned expected_growth = expected_size > wc->initial_size
        ? expected_size - wc->initial_size
        : 0U;
    int ok;
    ...
    if (expected_growth == 0U) {
        ...
        ok = observed_allocate.calls == 1U && observed_clear.calls == 1U &&
             observed_allocate.tb == node.file && observed_allocate.offset == wc->initial_size &&
             observed_allocate.len == expected_growth && observed_clear.node == &node &&
             observed_clear.start == wc->initial_size && observed_clear.len == expected_growth;
    }

    if (!ok) {
        fprintf(stderr,
                "growth mismatch: initial=%u expected_size=%u allocs=%u alloc=(%u,%u) clears=%u clear=(%u,%u)\n",
                wc->initial_size, expected_size, observed_allocate.calls,
                observed_allocate.offset, observed_allocate.len, observed_clear.calls,
                observed_clear.start, observed_clear.len);
    }
...
static enum theft_trial_res prop_file_write_matches_effective_write(struct theft *t, void *arg1)
{
    ...
    unsigned expected = expected_actual_len(wc->len, wc->offset);
    int ok;
    if (expected == 0U && wc->offset >= MAX_FILE_SIZE) {
        ok = actual == 0U && observed_write.calls == 0U;
    } else if (expected == 0U) {
        ok = actual == 0U &&
             (observed_write.calls == 0U ||
              (observed_write.calls == 1U && observed_write.tb == node.file &&
               observed_write.offset == wc->offset && observed_write.len == 0U &&
               observed_write.data == wc->buffer));
    } else {
...
int main(void)
{
    ...
    printf("inline_data baseline inode_write property-based tests:\n");
