# Bug: splitDirsFile does not terminate dirname output for empty paths (pre_alloc baseline)

## Status
Confirmed — failing theft property `terminates_uninitialized_output` against the real compiled `eval/pre_alloc/baseline/util.c::splitDirsFile` via CMake target `splitdirsfile_prealloc_baseline_pbt_test`.

## Property
`splitdirsfile_prealloc_baseline_pbt_test.terminates_uninitialized_output` (negative/error contract).
...
## Root Cause
`eval/pre_alloc/baseline/util.c` returns immediately when tokenization finds no components:
...
cmake --build /tmp/specfs-pbt-build --target splitdirsfile_prealloc_baseline_pbt_test -j2
ctest --test-dir /tmp/specfs-pbt-build -R '^splitdirsfile_prealloc_baseline_pbt_test$' --output-on-failure
